...
// Fetch details of provided branch on repository
//  Returns an Object
async function getBranchDetails(repoData, branch) {
  ...
  for await (const repo of repo_list) {
    ...
    for await (const branch of branches) {
      if (branchNameToCheck.toLowerCase === branch.name.toLowerCase) {
        // console.log('Fetching details for branch %s/%s/%s', orgName, repo.name, branch.name);
        const branch_details = await getBranchDetails(repo, branch.name);

        const branch_reply = {
          name: branch.name,
          protected: branch.protected,
          protection: branch_details.protection,
        };
